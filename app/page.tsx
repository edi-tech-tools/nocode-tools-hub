...
import {
  Search, Star, ArrowRight, Layers, Grid3X3,
  Bookmark, Clock, Users, ChevronRight, CheckCircle2,
  Sparkles, BarChart3, Globe, Layout
} from "lucide-react";
...
export default function HomePage() {
  ...
  const catColors = [
    { bg: "#1F1A2A", accent: "#EA580C" },
    { bg: "#1A1F30", accent: "#F97316" },
    { bg: "#1A2A1A", accent: "#10B981" },
    { bg: "#2A1A1A", accent: "#EF4444" },
    { bg: "#2A251A", accent: "#F59E0B" },
        ...
        <div className="relative max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
            {categories.slice(0, 6).map((cat, i) => {
              ...
              return (
                <Link key={cat.name} href={`/category/${cat.name.toLowerCase().replace(/ /g, "-")}`}
      ...
      <section className="px-6 py-4">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-2">
            <h2 className="text-lg font-bold text-white">{selectedCategory || "Editor&apos;s Picks"}</h2>
            <span className="text-xs text-gray-500 bg-[#1A1A2A] px-2 py-1 rounded-lg">{editorPicks.length} tools</span>
          </div>
              ...
              viewMode === "grid" ? (
                ...
                >
                  <div className="aspect-[4/3] bg-gradient-to-br flex items-center justify-center relative"
                    style={{ background: `linear-gradient(135deg, ${catColors[i % 6].accent}20, ${catColors[(i + 3) % 6].accent}10)` }}>
                    ...
                    {i < 2 && (
                      <span className="absolute top-2 right-2 text-[10px] px-2 py-0.5 rounded-full font-medium"
                        style={{ backgroundColor: `${ACCENT_COLOR}CC`, color: "white" }}>Editor&apos;s Pick</span>
                    )}
                  </div>
                  <div className="p-3">
                    ...
                    <div className="flex items-center justify-between mt-2">
                      ...
                      <span className="text-[10px] px-2 py-0.5 rounded bg-[#1A1A2A] text-gray-400">{tool.pricing || "$"}</span>
                    </div>
                  </div>
                </Link>
              ) : (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-2 text-[10px] text-gray-600">
                      ...
                      <span className="flex items-center gap-1"><Clock className="w-3 h-3" />{post.readTime || "3 min"}</span>
                    </div>
                  </div>
                </Link>
              ))}
        ...
        <div className="max-w-6xl mx-auto">
          <div className="relative rounded-2xl p-10 text-center overflow-hidden"
            ...
            <div className="relative">
              ...
              <h2 className="text-xl font-bold text-white mb-2">Start building without code</h2>
              <p className="text-sm text-gray-500 mb-6">Browse our full collection of curated no-code tools.</p>
              <Link href="/all-tools"
